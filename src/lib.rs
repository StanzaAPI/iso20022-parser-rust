//! Official Rust Client for ISO 20022 Financial Message Parser & Validator
...
#[derive(Debug, Clone)]
pub struct Iso20022ParserClient {
...
impl Iso20022ParserClient {
    pub fn new(api_key: Option<String>, base_url: Option<String>) -> Self {
        let key = api_key
            .or_else(|| env::var("STANZA_API_KEY").ok())
            .or_else(|| env::var("API_KEY").ok())
            .unwrap_or_default();
        let base = base_url.unwrap_or_else(|| "https://api.stanzaapi.com/iso20022-parser".to_string());
        ...
        Self {
            ...
            tool_url: "https://stanzaapi.com/tools/iso20022-parser",
        }
    }
    ...
    pub async fn parse<T: for<'de> Deserialize<'de>>(&self, payload: serde_json::Value) -> Result<ApiResponse<T>, reqwest::Error> {
        self.send_request("/api/v1/parse", reqwest::Method::POST, Some(payload)).await
    }
}
